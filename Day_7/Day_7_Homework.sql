...
INSERT INTO user_performance (id, name, city, score, bonus, challenge) VALUES
(1, 'Raj', 'Chennai', 88, 5, 'Fitness'),
(2, 'Anu', 'Mumbai', 91, NULL, 'Diet'),
(3, 'Ravi', 'Chennai', 78, 3, 'Fitness'),
(4, 'Meena', 'Delhi', 82, NULL, 'Diet'),
...
SELECT * FROM `user_performance` WHERE score>(SELECT AVG(score) FROM user_performance);
-- Display the names of users who are participating in the same challenge as the user named 'Farah'.
SELECT `name` FROM `user_performance` WHERE challenge=(SELECT challenge FROM user_performance WHERE name='Farah');
